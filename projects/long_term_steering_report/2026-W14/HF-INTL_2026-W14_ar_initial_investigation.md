...
**Overall:** Pre-Dunning Acceptance Rate for HF-INTL initial charges remained essentially flat at 90.13% (+0.01% WoW), a statistically non-significant change on 31,165 orders.
...
| L1: Country Impact | 6 countries exceeded ±2.5% threshold | LU -8.41%, DK -7.22%, AT -7.04%, DE -4.41%, AU -4.14%, CH +11.33% | ⚠️ |
| L1: Payment Method | Others +4.05%, all other methods within threshold | Minor shift | ✅ |
| L1: Payment Provider | All providers within ±2.5% threshold | Stable | ✅ |
| L2: Root Cause | Consistent pattern: "Insufficient Funds" increases across declining countries | +5.5 to +7.6pp | ⚠️ |
| L3: Related Metrics | All funnel metrics stable (FirstRunAR, PostDunningAR, PaymentApprovalRate) | <0.2% change | ✅ |
...
- **Insufficient Funds driving declines:** All four flagged countries (LU, DK, AT, DE) show "Insufficient Funds" as the primary decline reason, with increases of +7.61pp, +5.55pp, +5.67pp, and +2.51pp respectively
- **Payment method "None" anomaly:** Multiple countries show -100% change for "None" payment method, indicating a data categorization shift rather than a true operational issue
- **Adyen underperformance in smaller markets:** AT shows Adyen at -55.0% change (40.0% vs 88.89%), and DK shows Adyen at -10.64% change, though volumes are low (10 and 20 orders respectively)
- **CH positive outlier:** Switzerland improved +11.33% (77.27% → 86.03%) on 229 orders, partially offsetting declines elsewhere

**Action:** **Monitor** - The overall metric change is non-significant (+0.01%). Country-level declines are offset by improvements elsewhere. The "Insufficient Funds" pattern across multiple countries may warrant attention if trend continues next week, but no immediate escalation required.
