#!/usr/bin/env python3
"""
Run the post-spider-revamp notebook as a Databricks job
"""
...
def main():
    ...
    notebook_path = Path(__file__).parent / "[PY-3368] post-spider revamp"
    ...
    # Job configuration
    workspace_notebook_path = "/Workspace/Users/visal.kumar@hellofresh.com/[PY-3368] post-spider revamp"
    job_name = "Post-Spider Revamp - Customer Identifiers 20251121"
    
    print("=" * 80)
    print("Run Post-Spider Revamp Notebook as Job")
    print("=" * 80)
    ...
    if result:
        ...
        post_spider_files = [f for f in output_files if "post_spider_revamp" in f]
        
        if post_spider_files:
            print(f"\n✅ Found {len(post_spider_files)} output file(s):")
            for output_file in sorted(post_spider_files, reverse=True)[:2]:  # Show latest 2
                print(f"\n📄 Reading: {output_file}")
